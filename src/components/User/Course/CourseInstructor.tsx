...
export default function CourseInstructor({
  ...
  return (
    <Card className="rounded-2xl ring-1 ring-zinc-200/60 dark:ring-zinc-800/60">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 flex items-center gap-3 flex-wrap">
            <Space size={6}><Rate disabled allowHalf defaultValue={rating} /> <Text strong>{rating}</Text></Space>
      ...
      <Paragraph className="mt-4 text-gray-700 dark:text-gray-300">{bio}</Paragraph>

      <Space size={8} wrap className="mt-2">
        {socials.facebook && (
          <Button icon={<FacebookFilled />} href={socials.facebook} target="_blank">Facebook</Button>
        )}
        {socials.youtube && (
          <Button icon={<YoutubeFilled />} href={socials.youtube} target="_blank">YouTube</Button>
        )}
        {socials.linkedin && (
          <Button icon={<LinkedinFilled />} href={socials.linkedin} target="_blank">LinkedIn</Button>
        )}
        {socials.twitter && (
          <Button icon={<TwitterOutlined />} href={socials.twitter} target="_blank">Twitter</Button>
        )}
      </Space>
    </Card>
  );
