    ...
    <div className="mt-2 pl-10">
      <div className="flex items-start gap-3 rounded-lg bg-white/60 dark:bg-white/5 p-3 ring-1 ring-zinc-200/60 dark:ring-zinc-800/60">
        ...
        <div className="flex-1">
          ...
          <div className="text-[13px] text-gray-700 dark:text-gray-300">{r.content}</div>
        </div>
      </div>
    </div>
  );
...
type Props = {
  ...
  onSubmitReview?: (rating: number, text: string, files: UploadFile[]) => Promise<void> | void;
...
export default function CourseReviews({
  ...
  const filtered = useMemo(() => {
    ...
    if (sortBy === "highest") return [...base].sort((a, b) => b.rating - a.rating);
    if (sortBy === "lowest") return [...base].sort((a, b) => a.rating - b.rating);
  ...
  return (
    <Card className="rounded-2xl ring-1 ring-zinc-200/60 dark:ring-zinc-800/60 bg-cyan-50/80 dark:bg-cyan-950/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 items-start">
        <div className="md:col-span-1">
          <div className="rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800/60 p-4 sm:p-5 shadow-sm">
            <div className="text-base sm:text-lg font-semibold">{average} trên 5</div>
            <Rate disabled defaultValue={Math.round(average)} className="!mt-1" style={{ fontSize: 16 }} />
        ...
        <div className="md:col-span-2 space-y-2 sm:space-y-2.5">
          {[5, 4, 3, 2, 1].map((star, idx) => (
            <button key={star} onClick={() => onClickBar(star)} className="group w-full flex items-center gap-3">
              <div className="w-12 text-right text-sm tabular-nums">{star} Sao</div>
              <Progress
      ...
      {/* Write review */}
      <Card size="small" className="rounded-xl border border-zinc-200/60 dark:border-zinc-800/60 bg-white/90 dark:bg-zinc-900/90">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex justify-end">
            <Button type="primary" icon={<SendOutlined />} onClick={submitReview}>
              Gửi đánh giá
            </Button>
          </div>
        </div>
        ...
        <List
          ...
          renderItem={(item) => (
            <List.Item key={item.id} className="!px-0">
              ...
              />
              <Paragraph className="!mb-3 text-gray-700 dark:text-gray-300">{item.content}</Paragraph>

              {!!item.images?.length && (
                <div className="mb-3 grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {item.images.map((src, i) => (
                    <Image key={i} src={src} alt="review-photo" className="rounded-lg" />
                  ))}
                </div>
              )}

              <Space size="small" className="mb-2">
                <Button type="text" icon={<LikeOutlined />} onClick={() => markHelpful(item.id)}>
                  Hữu ích ({item.helpful})
                </Button>
                <Button type="text" icon={<MessageOutlined />}>
                  Bình luận
                </Button>
                <Button type="text" icon={<ExclamationCircleOutlined />} onClick={() => report(item.id)}>
                  Báo cáo
                </Button>
              </Space>

              {item.replies?.map((r) => <Reply key={r.id} r={r} />)}
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 flex justify-end">
                    <Button
                      ...
                      type="primary"
                      icon={<SendOutlined />}
                      onClick={() => message.success("Đã gửi bình luận (demo).")}
                    >
                      Gửi
                    </Button>
                  </div>
