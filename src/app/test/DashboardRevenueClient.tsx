// src/app/test/DashboardRevenueClient.tsx
"use client";
...
interface Props {
  initialRevenues: DailyRevenue[];
}

export default function DashboardRevenueClient({ initialRevenues }: Props) {
  // 1. Hydrate Zustand store
  useEffect(() => {
    usePaymentStore.setState({ revenues: initialRevenues });
  }, [initialRevenues]);

  // 2. Lấy dữ liệu từ store, theme & responsive
  const { revenues, isLoading, error } = usePaymentStore();
  const { token } = theme.useToken();
  const screens = useBreakpoint();
  const isMobile = !screens.md;

  // 3. Range picker state (chỉ để hiển thị)
  ...
  return (
    <>
      <Head>
        <title>EmoEase-Admin | Doanh thu</title>
      </Head>
      <BaseScreenAdmin
