...
const LivePreview: FC = () => {
  ...
  const totalDurationMinutes = curriculum.modules.reduce((acc, module) => acc + (module.duration || 0), 0);
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="pt-4 space-y-3">
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold text-gray-900 dark:text-white">{formatPrice(pricing.discountPrice || pricing.basePrice)}</span>
            {pricing.discountPrice && (
              <span className="text-lg text-gray-400 line-through">{formatPrice(pricing.basePrice)}</span>
            )}
          </div>
          {pricing.publicPromoMessage && (
            <div className="flex items-center gap-2 text-sm text-yellow-600 dark:text-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 px-3 py-1.5 rounded-md">
              <FaBullhorn />
              <span>{pricing.publicPromoMessage}</span>
            </div>
          )}
        ...
        <div className="pt-4">
          ...
          <div className="space-y-2">
            {curriculum.modules.slice(0, 3).map((module, index) => (
              <div key={module.id} className="text-sm bg-gray-50 dark:bg-gray-700/50 p-3 rounded-md flex justify-between">
                <span><strong>Chương {index + 1}:</strong> {module.title}</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">{module.duration} phút</span>
              </div>
            ))}
            {curriculum.modules.length > 3 && (
              <div className="text-center text-sm text-gray-500 dark:text-gray-400">... và {curriculum.modules.length - 3} chương nữa</div>
            )}
          </div>
        </div>
      </div>
